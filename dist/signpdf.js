...
class SignPdf {
  ...
  signPdfReady({
    path_to_p12_certificate = `${__dirname}/helpers/mobifone.p12`,
    pass_p12_certificate = '123123',
    filePath = `${__dirname}/helpers/w3dummy.pdf`,
    storagePath = __dirname,
    filename = Date.now() + '-pdf-signed.pdf',
    reason = 'Sign PDF',
    signatureInfo = {
      email: 'cuongdq.payment@gmail.com',
      location: 'Da Nang, Vietnam',
      name: 'c3.mobifone.vn'
    }
  } = {}) {
    const p12Buffer = _fs.default.readFileSync(path_to_p12_certificate);

    let pdfBuffer = _fs.default.readFileSync(filePath);

    pdfBuffer = (0, _helpers.plainAddPlaceholder)({
      pdfBuffer,
      reason,
      signatureLength: 7448,
      signatureInfo
    });
    pdfBuffer = this.sign(pdfBuffer, p12Buffer, {
      passphrase: pass_p12_certificate
    });

    _fs.default.writeFileSync(storagePath + '/' + filename, pdfBuffer);
  }
